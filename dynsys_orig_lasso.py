...
def dynSys(var_dat=None,epoch_dat=None,region_dat=None,sampling_time=.0025):
    ...
    for k in range(len(epoch_dat)):
        ...
        for j in range(x.shape[1]):
            sin_terms = np.sin(x[:-1, j])
            cos_terms = np.cos(x[:-1, j])
            sin2_terms = np.sin(2 * x[:-1, j])
            cos2_terms = np.cos(2 * x[:-1, j])
            
            phix_list.append(sin_terms)
            phix_list.append(cos_terms)
            phix_list.append(sin2_terms)
            phix_list.append(cos2_terms)
        ...
        condition_numbers.append(np.linalg.cond(W))
        #print(W.shape)
